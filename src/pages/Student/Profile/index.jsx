...
const LEVEL_CONFIG = {
    ...
    N4: { light: 'bg-emerald-50 text-emerald-700 ring-emerald-200', bar: 'bg-emerald-500', color: '#10b981' },
    N3: { light: 'bg-violet-50 text-violet-700 ring-violet-200', bar: 'bg-violet-500', color: '#8b5cf6' },
    N2: { light: 'bg-amber-50 text-amber-700 ring-amber-200', bar: 'bg-amber-500', color: '#f59e0b' },
...
const SECTION_COLOR = { vocabulary: '#0ea5e9', grammar: '#8b5cf6', reading: '#10b981', listening: '#f59e0b' }
    ...
    return (
        <button
            ...
            className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all ${
                active ? 'bg-primary text-white shadow-md' : 'bg-surface text-text-light hover:bg-border/30'
            }`}
        >
            {children}
        </button>
...
export default function ProfilePage() {
    ...
    useEffect(() => { loadProfile(); loadStats(); loadAchievements(); loadStudyGoal() }, [])
    ...
    const ICON_MAP = {
        rocket: Zap, swords: Award, flame: Flame, zap: Zap,
        'calendar-check': Calendar, 'calendar-heart': Calendar, crown: Trophy,
        'check-circle': CheckCircle2, award: Award, star: Star, medal: Trophy,
    }
                ...
                {loading ? (
                    ...
                    <div className="mt-8 space-y-8">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-4">
                            <StatCard icon={Flame} label="Streak" value={`${streak} ngày`}
                                gradient="bg-linear-to-br from-orange-400 to-red-500" iconBg="bg-white/20 text-white" />
                                ...
                                <div className="grid sm:grid-cols-2 gap-4">
                                    <div className="space-y-2">
                                        <div className="flex justify-between text-xs font-semibold">
                                            ...
                                            <span className="text-primary">{studyGoal.progress?.daily?.questionsAnswered || 0} / {studyGoal.progress?.daily?.target || 20}</span>
                                        </div>
                                        <div className="h-3 rounded-full bg-primary/10 overflow-hidden">
                                            <div className="h-full rounded-full bg-linear-to-r from-primary to-cta transition-all duration-700"
                                                style={{ width: `${Math.min(100, ((studyGoal.progress?.daily?.questionsAnswered || 0) / (studyGoal.progress?.daily?.target || 20)) * 100)}%` }} />
                                        </div>
                                    </div>
                                    <div className="space-y-2">
                                        <div className="flex justify-between text-xs font-semibold">
                                            <span className="text-text-light">Bài thi tuần này</span>
                                            <span className="text-cta">{studyGoal.progress?.weekly?.examsCompleted || 0} / {studyGoal.progress?.weekly?.target || 3}</span>
                                        </div>
                                        <div className="h-3 rounded-full bg-cta/10 overflow-hidden">
                                            <div className="h-full rounded-full bg-linear-to-r from-cta to-emerald-500 transition-all duration-700"
                                                style={{ width: `${Math.min(100, ((studyGoal.progress?.weekly?.examsCompleted || 0) / (studyGoal.progress?.weekly?.target || 3)) * 100)}%` }} />
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            ...
                            <TabButton active={activeTab === 'charts'} onClick={() => setActiveTab('charts')}>Biểu đồ</TabButton>
                            <TabButton active={activeTab === 'history'} onClick={() => setActiveTab('history')}>Lịch sử</TabButton>
                            <TabButton active={activeTab === 'achievements'} onClick={() => setActiveTab('achievements')}>Thành tựu ({earnedCount}/{achievements.length})</TabButton>
                        </div>
                        ...
                        {activeTab === 'overview' && (
                            <div className="space-y-8">
                                ...
                                {comparison && (
                                    <div className="rounded-2xl border border-border bg-white p-5 shadow-sm">
                                        <h3 className="text-sm font-bold text-text mb-4 flex items-center gap-2">
                                            <TrendingUp className="size-4 text-primary" />So sánh 2 lần thi gần nhất
                                        </h3>
                                        <div className="grid sm:grid-cols-2 gap-4">
                                            {comparison.map((att, idx) => {
                                                const isLatest = idx === 0
                                                return (
                                                    <div key={att._id} className={`rounded-xl p-4 ${isLatest ? 'bg-primary/5 border border-primary/20' : 'bg-surface border border-border'}`}>
                                                        <div className="flex items-center justify-between mb-2">
                                                            <span className={`text-xs font-bold ${isLatest ? 'text-primary' : 'text-text-muted'}`}>
                                                                {isLatest ? 'Lần mới nhất' : 'Lần trước'}
                                                            </span>
                                                            <span className="text-xs text-text-muted">{formatDateShort(att.startTime)}</span>
                                                        </div>
                                                        <p className="text-2xl font-black text-text">{att.results?.percentage || 0}%</p>
                                                        <p className="text-xs text-text-muted">
                                                            {att.results?.totalScore}/{att.results?.maxScore} điểm · {formatDuration(att.duration)}
                                                        </p>
                                                        {att.results?.sectionScores?.map(ss => (
                                                            <div key={ss.sectionType} className="mt-2">
                                                                <div className="flex justify-between text-xs">
                                                                    <span className="text-text-muted">{SECTION_NAME[ss.sectionType] || ss.sectionType}</span>
                                                                    <span className="font-semibold text-text">{ss.correctAnswers}/{ss.totalQuestions}</span>
                                                                </div>
                                                            </div>
                                                        ))}
                                                    </div>
                                        ...
                                        {comparison.length === 2 && (
                                            <div className="mt-3 text-center">
                                                {comparison[0].results?.percentage > comparison[1].results?.percentage ? (
                                                    <span className="text-xs font-semibold text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full">
                                                        Tiến bộ +{comparison[0].results.percentage - comparison[1].results.percentage}%
                                                    </span>
                                                ) : comparison[0].results?.percentage < comparison[1].results?.percentage ? (
                                                    <span className="text-xs font-semibold text-red-500 bg-red-50 px-3 py-1 rounded-full">
                                                        Giảm {comparison[1].results.percentage - comparison[0].results.percentage}%
                                                    </span>
                                                ) : (
                        ...
                        {activeTab === 'charts' && (
                            <div className="space-y-8">
                                ...
                                {dailyChartData.length > 0 && (
                                    <div className="rounded-2xl border border-border bg-white p-5 shadow-sm">
                                        <h3 className="text-sm font-bold text-text mb-4 flex items-center gap-2">
                                            <TrendingUp className="size-4 text-primary" />Điểm theo ngày (30 ngày gần nhất)
                                        </h3>
                                        <ResponsiveContainer width="100%" height={280}>
                                            <AreaChart data={dailyChartData}>
                                                ...
                                                <Area type="monotone" dataKey="Trung bình" stroke="#6366f1" fill="url(#colorAvg)" strokeWidth={2} />
                                                <Area type="monotone" dataKey="Cao nhất" stroke="#10b981" fill="url(#colorBest)" strokeWidth={2} />
                                            </AreaChart>
                                        </ResponsiveContainer>
                                    </div>
                                )}
                                ...
                                {sectionChartData.length > 0 && (
                                    <div className="rounded-2xl border border-border bg-white p-5 shadow-sm">
                                        <h3 className="text-sm font-bold text-text mb-4 flex items-center gap-2">
                                            <Sparkles className="size-4 text-primary" />Điểm trung bình theo kỹ năng
                                        </h3>
                                        <ResponsiveContainer width="100%" height={300}>
                                            <RadarChart data={sectionChartData}>
                                                ...
                                                <Radar name="Điểm TB" dataKey="score" stroke="#6366f1" fill="#6366f1" fillOpacity={0.3} strokeWidth={2} />
                                            </RadarChart>
                                        </ResponsiveContainer>
                                    </div>
                                )}
                                ...
                                {accuracyData.length > 0 && (
                                    <div className="rounded-2xl border border-border bg-white p-5 shadow-sm">
                                        <h3 className="text-sm font-bold text-text mb-4 flex items-center gap-2">
                                            <BarChart3 className="size-4 text-primary" />Tỷ lệ đúng/sai theo cấp độ
                                        </h3>
                                        <ResponsiveContainer width="100%" height={280}>
                                            <BarChart data={accuracyData}>
                                                ...
                                                <Bar dataKey="Sai" fill="#f43f5e" radius={[4, 4, 0, 0]} />
                                                <Bar dataKey="Bỏ qua" fill="#94a3b8" radius={[4, 4, 0, 0]} />
                                            </BarChart>
                                        </ResponsiveContainer>
                                    </div>
                                )}
                                ...
                                {dailyChartData.length === 0 && sectionChartData.length === 0 && (
                                    <div className="rounded-2xl border-2 border-dashed border-border bg-white p-16 text-center">
                                        ...
                                        <p className="font-semibold text-text-muted">Chưa có dữ liệu biểu đồ</p>
                                        <p className="text-sm text-text-muted/70">Hoàn thành ít nhất 1 bài thi để xem biểu đồ tiến bộ.</p>
                                    </div>
                                )}
                            </div>
                        )}
                        ...
                        {activeTab === 'achievements' && (
                            <div>
                                <h2 className="text-base font-bold text-text mb-4 flex items-center gap-2">
                                    <Trophy className="size-4 text-amber-500" />Thành tựu ({earnedCount}/{achievements.length})
                                </h2>
                                {achievements.length === 0 ? (
                                    <div className="rounded-2xl border-2 border-dashed border-border bg-white p-16 text-center">
                                        <Trophy className="size-10 text-text-muted/30 mx-auto mb-3" />
                                        <p className="font-semibold text-text-muted">Chưa có thành tựu nào</p>
                                    </div>
                                ) : (
                                    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                                ...
                                                >
                                                    <div className="flex items-start gap-3">
                                                        ...
                                                        <div className="min-w-0">
                                                            <p className={`font-bold text-sm ${ach.earned ? 'text-text' : 'text-text-muted'}`}>{ach.titleVi}</p>
                                                            <p className="text-xs text-text-muted mt-0.5">{ach.descriptionVi}</p>
                                                            {ach.earned && ach.earnedAt && (
                                                                <p className="text-[10px] text-amber-600 font-semibold mt-1">
                                                                    Đạt được {formatDateShort(ach.earnedAt)}
                                                                </p>
                                                            )}
                                                        </div>
                                                    </div>
