    ...
    return (
        <div className="space-y-8">
            <div>
                ...
                <p className="mt-1 text-sm text-text-light">Tham khảo cấu trúc chuẩn JLPT kết hợp thống kê đề thi thực tế trong hệ thống.</p>
            </div>
                        ...
                        <div key={structure.id} className="group rounded-2xl border border-border bg-white shadow-sm overflow-hidden transition hover:shadow-md">
                                ...
                                <div className="flex items-center justify-between">
                                    <div>
                                        ...
                                        <div className="mt-1 flex items-center gap-3 text-sm text-white/80">
                                            <span className="flex items-center gap-1"><Clock className="size-3.5" /> {structure.duration} phút</span>
                                            <span className="flex items-center gap-1"><CheckCircle2 className="size-3.5" /> {structure.totalQuestions} câu</span>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-3xl font-black text-white">{examCount}</p>
                            ...
                            <div className="p-5 space-y-3">
                                {structure.sections.map((section, i) => {
                                    ...
                                    const pct = Math.round((section.time / structure.duration) * 100)
                                    return (
