...
const STATUS_MAP = {
    ...
    published: { label: 'Đã xuất bản', cls: 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200' },
...
export default function AdminExamsPage() {
    ...
    useEffect(() => {
        const h = e => { if (menuRef.current && !menuRef.current.contains(e.target)) setActionMenu(null) }
        document.addEventListener('mousedown', h)
        return () => document.removeEventListener('mousedown', h)
    }, [])

    const handlePublish = async (examId) => {
        try { await examService.publishExam(examId); load() } catch {}
        setActionMenu(null)
    }

    const handleDelete = async (examId) => {
        if (!confirm('Xác nhận xóa đề thi này?')) return
        try { await examService.deleteExam(examId); load() } catch {}
        setActionMenu(null)
    }
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <span className="text-sm font-semibold text-primary bg-primary/10 px-3 py-1.5 rounded-full">{total} đề thi</span>
            </div>
            ...
            <div className="flex flex-wrap gap-3">
                <div className="relative max-w-xs flex-1">
                    ...
                    <input type="text" placeholder="Tìm theo tên, mã đề..." value={search}
                        ...
                        <table className="w-full text-left text-sm">
                            <thead>
                                <tr className="border-b border-border bg-surface/50">
                                    <th className="px-5 py-3 font-semibold text-text-muted w-12">#</th>
                                    <th className="px-5 py-3 font-semibold text-text-muted">Tên đề thi</th>
                                    <th className="px-5 py-3 font-semibold text-text-muted">Level</th>
                                    <th className="px-5 py-3 font-semibold text-text-muted">Trạng thái</th>
                                    <th className="px-5 py-3 font-semibold text-text-muted">Người tạo</th>
                                    <th className="px-5 py-3 font-semibold text-text-muted">Lượt thi</th>
                                    <th className="px-5 py-3 font-semibold text-text-muted">Ngày tạo</th>
                                    <th className="px-5 py-3 font-semibold text-text-muted w-16"></th>
                                </tr>
                            </thead>
                            <tbody>
                                {exams.length === 0 ? (
                                    <tr><td colSpan={8} className="px-5 py-12 text-center text-text-muted">Không tìm thấy đề thi nào.</td></tr>
                                ) : exams.map((ex, idx) => {
                                    const st = STATUS_MAP[ex.status] || STATUS_MAP.draft
                                    return (
                                        <tr key={ex._id} className="border-b border-border/50 hover:bg-background/50 transition">
                                            <td className="px-5 py-3 text-text-muted">{start + idx + 1}</td>
                                            <td className="px-5 py-3">
                                                <div>
                                                    <p className="font-medium text-text">{ex.title || '-'}</p>
                                                    <p className="text-xs text-text-muted">{ex.examCode || ''}</p>
                                                </div>
                                            </td>
                                            <td className="px-5 py-3">
                                                <span className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-black ${LEVEL_BADGE[ex.level] || 'bg-gray-100 text-gray-600'}`}>
                                                    {ex.level}
                                                </span>
                                            </td>
                                            <td className="px-5 py-3">
                                                <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold ${st.cls}`}>
                                                    {ex.isPublic && <Globe className="size-3" />}
                                                    {st.label}
                                                </span>
                                            </td>
                                            <td className="px-5 py-3 text-text-light">{ex.createdBy?.fullName || '-'}</td>
                                            <td className="px-5 py-3">
                                                <div className="flex items-center gap-2 text-text-muted">
                                                    <Eye className="size-3.5" />{ex.viewCount || 0}
                                                    <span className="text-border">|</span>
                                                    <BookOpen className="size-3.5" />{ex.attemptCount || 0}
                                                </div>
                                            </td>
                                            <td className="px-5 py-3 text-text-muted">{formatDate(ex.createdAt)}</td>
                                            <td className="px-5 py-3 relative">
                                                <button onClick={() => setActionMenu(actionMenu === ex._id ? null : ex._id)}
                                                    className="p-1.5 rounded-lg hover:bg-surface transition cursor-pointer">
                                                    <MoreHorizontal className="size-4 text-text-muted" />
                                                </button>
                                                {actionMenu === ex._id && (
                                                    <div ref={menuRef} className="absolute right-5 top-10 z-50 w-48 bg-white rounded-xl border border-border shadow-lg py-1">
                                                        <button onClick={() => { navigate(`/creator/exam/${ex._id}/questions`); setActionMenu(null) }}
                                                            className="flex items-center gap-2 w-full px-4 py-2.5 text-sm hover:bg-surface transition text-left cursor-pointer">
                                                            <Eye className="size-4 text-primary" />Xem chi tiết
                                                        </button>
                                                        {ex.status === 'draft' && (
                                                            <button onClick={() => handlePublish(ex._id)}
                                                                className="flex items-center gap-2 w-full px-4 py-2.5 text-sm hover:bg-surface transition text-left cursor-pointer">
                                                                <Send className="size-4 text-emerald-600" />Xuất bản
                                                            </button>
                                                        )}
                    ...
                    {totalPages > 1 && (
                        <div className="flex items-center justify-between gap-4 border-t border-border px-5 py-3">
                            ...
                            <div className="flex items-center gap-1">
                                ...
                                <span className="px-3 py-1.5 text-sm text-text-light">Trang {page} / {totalPages}</span>
